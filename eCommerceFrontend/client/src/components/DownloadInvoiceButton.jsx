...
export default function DownloadInvoiceButton({ order }) {
  const handleDownload = async () => {
    ...
    const productImages = await Promise.all(
      order.items.map((item) => toBase64(item.product?.image))
    );
    ...
    doc.text(`${order.offerPrice}`, valueX, textY, { align: "right" });

    doc.setFont("helvetica", "bold");
    doc.setTextColor(40);
    doc.text("Total Amount:", labelX, textY + 8);
    doc.setTextColor(220, 53, 69);
    doc.text(`${order.totalAmount}`, valueX, textY + 8, { align: "right" });
